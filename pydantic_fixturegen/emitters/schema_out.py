...
def emit_model_schema(
    ...
    *,
    ...
    config = SchemaEmitConfig(output_path=Path(output_path), indent=_normalise_indent(indent), ensure_ascii=ensure_ascii)
    schema = model.model_json_schema()
    payload = json.dumps(schema, indent=config.indent, ensure_ascii=config.ensure_ascii, sort_keys=True)
...
def emit_models_schema(
    ...
    *,
    ...
    payload = json.dumps(combined, indent=config.indent, ensure_ascii=config.ensure_ascii, sort_keys=True)
    config.output_path.parent.mkdir(parents=True, exist_ok=True)
    config.output_path.write_text(payload, encoding="utf-8")
    return config.output_path
