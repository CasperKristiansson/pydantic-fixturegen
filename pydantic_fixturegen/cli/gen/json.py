...
def register(app: typer.Typer) -> None:
    ...
    def gen_json(  # noqa: PLR0913 - CLI surface mirrors documented parameters
        ...
        if include:
            cli_overrides["include"] = _split_patterns(include)
        if exclude:
            cli_overrides["exclude"] = _split_patterns(exclude)
        ...
        try:
            model_cls = _load_model_class(target_model)
...
def _load_model_class(model_info: IntrospectedModel) -> type[BaseModel]:
    module = _load_module(model_info)
    name = model_info.name
    attr = getattr(module, name, None)
    if not isinstance(attr, type) or not issubclass(attr, BaseModel):
        raise RuntimeError(f"Attribute {name!r} in module {module.__name__} is not a Pydantic BaseModel.")
    return attr


def _load_module(model_info: IntrospectedModel) -> ModuleType:
    module_name = model_info.module
    locator = Path(model_info.locator)
